...
MonodroidRuntime::open_from_update_dir (MonoAssemblyName *aname, char **assemblies_path, void *user_data)
{
	...
	if (inMemoryAssemblies.has_assemblies ()) {
		...
		result = inMemoryAssemblies.try_load_assembly (domain, aname);
		if (result != nullptr) {
...
#ifndef ANDROID
	if (inMemoryAssemblies.has_assemblies () && inMemoryAssemblies.try_load_assembly (domain, aname) != nullptr) {
                                                ...
                                                jobject loader, bool is_root_domain, bool force_preload_assemblies)
{
	...
	if (assembliesBytes != nullptr)
		inMemoryAssemblies.add_or_update_from_java (domain, env, assemblies, assembliesBytes, assembliesPaths);
#endif
