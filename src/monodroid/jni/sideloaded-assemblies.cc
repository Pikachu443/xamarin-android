#include "sideloaded-assemblies.hh"
#include "globals.hh"
...
void
SideloadedAssemblies::add_or_update_from_java (MonoDomain *domain, JNIEnv *env, jstring_array_wrapper &assemblies, jobjectArray assembliesBytes, jstring_array_wrapper &assembliesPaths)
{
	assert (assembliesBytes != nullptr);
	SideloadedAssemblyEntry *new_entry = new SideloadedAssemblyEntry (domain, env, assemblies, assembliesBytes, assembliesPaths);
	add_or_replace_entry (new_entry);
}

MonoAssembly*
SideloadedAssemblies::try_load_assembly (MonoDomain *domain, MonoAssemblyName *name)
{
	int domain_id = mono_domain_get_id (domain);
	SideloadedAssemblyEntry *entry = find_entry (domain_id);
	if (entry == nullptr)
		return nullptr;
...
void
SideloadedAssemblies::clear_for_domain (MonoDomain *domain)
{
	int domain_id = mono_domain_get_id (domain);
	SideloadedAssemblyEntry *entry = remove_entry (domain_id);
	delete entry;
}

SideloadedAssemblies::SideloadedAssemblyEntry*
SideloadedAssemblies::find_entry (int domain_id)
{
...
void
SideloadedAssemblies::add_or_replace_entry (SideloadedAssemblies::SideloadedAssemblyEntry *new_entry)
{
	for (unsigned int i = 0; i < length; i++) {
		auto entry = entries[i];
...
void
SideloadedAssemblies::add_entry (SideloadedAssemblies::SideloadedAssemblyEntry *entry)
{
	if (length >= capacity) {
		capacity = MULTIPLY_WITH_OVERFLOW_CHECK(unsigned int, capacity, 2);
		SideloadedAssemblyEntry **new_entries = new SideloadedAssemblyEntry*[capacity];
		memcpy (new_entries, entries, MULTIPLY_WITH_OVERFLOW_CHECK(size_t, sizeof(void*), length));
		SideloadedAssemblyEntry **old_entries = entries;
		entries = new_entries;
		delete[] old_entries;
	}
...
SideloadedAssemblies::SideloadedAssemblyEntry*
SideloadedAssemblies::remove_entry (int domain_id)
{
	for (unsigned int i = 0; i < length; i++) {
		SideloadedAssemblyEntry *entry = entries[i];
		if (entry->domain_id == domain_id) {
...
SideloadedAssemblies::SideloadedAssemblyEntry::SideloadedAssemblyEntry (MonoDomain *domain, JNIEnv *env, jstring_array_wrapper &assemblies, jobjectArray assembliesBytes, jstring_array_wrapper &assembliesPaths)
{
...
SideloadedAssemblies::SideloadedAssemblyEntry::~SideloadedAssemblyEntry ()
{
